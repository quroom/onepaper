<template>
  <v-container v-if="!isLoading">
    ...
    <div class="mt-3">
      1. {{ $t("desc_realestate") }} {{ $t("and") }} {{ $t("mandate") }}
      {{ $t("period") }}
    </div>
    <ValidationObserver ref="mandate_obs">
      ...
      <div class="mt-3">
        2. {{ $t("designator") }} {{ $t("and") }} {{ $t("designee") }}
        {{ $t("info") }}
      </div>
        ...
        <v-row>
          <v-col cols="12" md="6">
            ...
            <v-expansion-panel v-if="designator">
              <v-expansion-panel-header
                >{{ $t("designator") }} {{ $t("detail") }} {{ $t("info") }}
                {{ `(${designator.user.email})` }}</v-expansion-panel-header
              >
          ...
          <v-col cols="12" md="6">
            ...
            <v-expansion-panel v-if="designee">
              <v-expansion-panel-header
                >{{ $t("designee") }} {{ $t("detail") }} {{ $t("info") }}
                {{ `(${designee.user.email})` }}</v-expansion-panel-header
              >
              <v-expansion-panel-content>
                <ContractorItem
                  :profile="designee"
      ...
      <div class="mt-3" align="end" style="position:relative;">
        <img
          v-if="designator_signature_url"
          class="signature-img"
          :src="designator_signature_url"
        />
...
export default {
  ...
  data() {
    return {
      ...
      panels: [0, 1],
      content:
        "제1조 상기 위임인은 수임인에게 부동산 기본정보에 기재된 거래 대상 부동산의 계약에 관한 사무를 위임한다.<br>제2조 상기 위임인은 거래계약체결 상대방에게 위임사실을 알리기 위하여 위임장 사본을 제공한다.",
