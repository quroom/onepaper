  ...
  <ValidationObserver ref="obs">
    <v-container>
      ...
      <template v-if="is_expert">
        ...
        <v-row>
          <v-col
            class="d-flex child-flex img-col"
            cols="6"
            md="3"
          >
            <div class="absolute_text">{{ $t("registration_certificate") }}</div>
            <a v-if="current_registration_certificate" v-bind:href="current_registration_certificate" target="_blank">
              <img
                class="img"
          ...
          <v-col class="d-flex child-flex img-col" cols="6" md="3">
            ...
            <a v-if="current_garantee_insurance" v-bind:href="current_garantee_insurance" target="_blank">
      ...
      <v-dialog v-if="dialog.flag&&dialog.insurances" v-model="dialog.flag" max-width="750px">
        <v-card>
            ...
            <v-row justify="center">
              <v-col cols="auto" v-for="insurance in dialog.insurances" :key="insurance.id">
                <v-card>
                  ...
                  <v-card-subtitle>
                    <div>{{`${$t("garantee_insurance")} ${ $t("period") }`}}</div>
                    {{ insurance.from_date }} ~ {{ insurance.to_date }}
                  </v-card-subtitle>
                  <v-card-actions>
                    <DeleteAlert :id="insurance.id" :callback="deleteInsurance"></DeleteAlert>
                    <v-spacer></v-spacer>
                    <v-btn class="ma-1 auto no-print" color="green" dark @click="loadInsurnace(insurance.id)">
                      {{$t('edit')}}
                    </v-btn>
                  </v-card-actions>
                </v-card>
          ...
          <v-row no-gutters>
            <v-col cols="12">
              <v-btn v-if="dialog.insurance.id" class="ml-2 mr-4 mb-4" outlined dark color="red" @click="init_insurance">
                {{$t("initialize")}}
              </v-btn>
              <v-btn v-if="dialog.insurance.id"  class="mr-4 mb-4 right-btn" color="primary" dark @click="submitInsurance">
                {{`${$t("edit")} ${$t("submit")}`}}
              </v-btn>
              <v-btn v-else class="mr-4 mb-4 right-btn" color="primary" dark @click="submitInsurance">
                <v-icon>add</v-icon>
                {{$t("add_garantee_insurance")}}
              </v-btn>
            </v-col>
      ...
      if (data.id != undefined) {
        if (data.user.is_expert) {
          return next((vm) => {
            ...
            vm.dates = [data.expert_profile.insurance.from_date, data.expert_profile.insurance.to_date];
