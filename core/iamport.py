...
class Iamport(object):
    def __init__(self, imp_url=IAMPORT_API_URL):
        self.imp_key = os.environ.get("IMP_KEY")
        self.imp_secret = os.environ.get("IMP_SECRET")
    ...
    def get_response(response):
        ...
        if response.status_code != requests.codes.ok:
            raise Iamport.HttpError(response.status_code, result.get("message"))
        if result["code"] != 0:
            raise Iamport.ResponseError(response.status_code, result.get("message"))
