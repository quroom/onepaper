...
class VerifyingExplanationSerializer(serializers.ModelSerializer):
    ...
    explanation_evidences = fields.MultipleChoiceField(
        choices=VerifyingExplanation.EXPLANATION_EVIDENCE_CATEGORY
    )
...
class PaperListSerializer(ReadOnlyModelSerializer):
    ...
    def get_answer_count(self, instance):
        return instance.paper_contractors.get(
            profile__user=self.context["request"].user
        ).contractor_answers.count()
...
class PaperSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        verifying_explanation_data = (
            validated_data.pop("verifying_explanation")
            if not validated_data.get("verifying_explanation") is None
            else None
        )
        ...
        for contractor_data in contractors_data:
            Contractor.objects.create(
                profile=contractor_data["profile"],
                paper=paper,
                group=contractor_data["group"],
                is_allowed=contractor_data["is_allowed"],
            )
            if (
                status_instance.status != PaperStatus.REQUESTING
                and contractor_data["is_allowed"] == None
            ):
                status_instance.status = PaperStatus.REQUESTING
        ...
        if self.context["request"].user.is_expert == True:
            ...
            VerifyingExplanation.objects.create(
                paper=paper, address=address, **verifying_explanation_data
            )
        return paper
    ...
    def update(self, instance, validated_data):
        ...
        contractors_data = (
            validated_data.pop("paper_contractors")
            if not validated_data.get("paper_contractors") is None
            else {}
        )
        verifying_explanation_data = (
            validated_data.pop("verifying_explanation")
        ...
        for contractor_data in contractors_data:
            try:
                ...
                try:
                    ...
                    if (
                        contractor_data["is_allowed"] == None
        ...
        if self.context["request"].user.is_expert == True:
            ...
            verifying_explanation_address_data = (
                verifying_explanation_data.pop("address")
                if not verifying_explanation_data.get("address") is None
                else {}
            )
    ...
    def validate(self, data):
        ...
        expert_profile = None
        contractors = (
            data["paper_contractors"] if not data.get("paper_contractors") is None else []
        )
        users_id_list = []
        for index, contractor in enumerate(contractors):
            ...
            if author != contractor["profile"].user:
                if not AllowedUser.objects.filter(
                    allowed_users=author, profile=contractor["profile"]
                ).exists():
        ...
        if self.context["request"].method in ["PUT", "POST"]:
            ...
            if not author.id in users_id_list:
                raise serializers.ValidationError(
                    {"seller": _("작성자가 포함되지 않았습니다."), "buyer": _("작성자가 포함되지 않았습니다.")}
                )
        return data
...
class PaperReadonlySerializer(ReadOnlyModelSerializer):
    ...
    def get_mandates(self, instance):
        ...
        mandates = Mandate.objects.filter(
            address__old_address=instance.address.old_address,
            designee__user=instance.author,
...
class PaperEveryoneSerializer(ReadOnlyModelSerializer):
    ...
    def get_address(self, instance):
        if instance.is_contractor:
            return {
                "old_address": instance.address.old_address,
                "old_address_eng": instance.address.old_address_eng,
                "dong": instance.address.dong,
                "ho": instance.address.ho,
            }
        address_with_bun = instance.address.old_address.split("-")[0]
        hidden_address = address_with_bun[0 : address_with_bun.rindex(" ")]
        old_address_eng = instance.address.old_address_eng
