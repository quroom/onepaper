...
from papers.serializers import (
    ...
    PaperEveryoneSerializer,
    PaperListSerializer,
    PaperLoadSerializer,
    PaperReadonlySerializer,
    PaperSerializer,
    PaperUnalloweUserDetailSerializer,
    PaperUnalloweUserSerializer,
    SignatureSerializer,
)
...
class PaperFilter(django_filters.FilterSet):
    ...
    old_address = django_filters.CharFilter(
        lookup_expr="icontains", field_name="address__old_address"
    )
                    ...
                    When(
                        Exists(
                            Contractor.objects.filter(
                                paper=OuterRef("pk"), profile__user=self.request.user
                            )
                        ),
                        then=True,
                    ),
...
class AllowPaperAPIView(APIView):
    ...
    def put(self, request, pk):
        ...
        if is_allowed == None:
            return Response(
                {"detail": ValidationError(_("요청 데이터가 올바르지 않습니다."))},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if contractor.paper.status.status in (PaperStatus.PROGRESS, PaperStatus.DONE):
            return Response(
                {"detail": ValidationError(_("서명 또는 완료된 계약서는 거절할 수 없습니다."))},
                status=status.HTTP_400_BAD_REQUEST,
            )
        else:
            ...
            if is_allowed == True:
                if contractor.is_allowed == True:
                    return Response(
                        {"detail": ValidationError(_("이미 계약서 작성이 허용 되었습니다."))},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                else:
                ...
                if contractor.is_allowed == False:
                    return Response(
                        {"detail": ValidationError(_("이미 계약서 작성이 거절 되었습니다."))},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                else:
    ...
    def put(self, request, pk):
        ...
        if contractor.paper.status.status in (PaperStatus.DENIED, PaperStatus.DONE):
            if is_hidden:
                if contractor.is_hidden:
                    return Response(
                        {"detail": ValidationError(_("이미 계약서가 숨김 처리 되었습니다."))},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                else:
                ...
                if contractor.is_hidden == False:
                    return Response(
                        {"detail": ValidationError(_("이미 계약서가 보임 처리 되었습니다."))},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                else:
            ...
            return Response(
                {"detail": ValidationError(_("완료 또는 거절된 계약서만 숨김 처리가 가능합니다."))},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class ExplanationSignatureCreateAPIView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if paper.status.status in (PaperStatus.DONE, PaperStatus.DENIED):
            return Response(
                {"detail": ValidationError(_("완료 또는 거절된 계약서는 서명을 추가할 수 없습니다."))},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if explanation_signatures.exists():
            ...
            if explanation_signature.updated_at >= paper.updated_at:
                return Response(
                    {"detail": ValidationError(_("이미 서명이 등록되어있습니다."))},
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class ExplanationSignatureUpdateAPIView(
    ...
    def put(self, request, *args, **kwargs):
        ...
        if explanation_signature.contractor.paper.status.status in (
            PaperStatus.DONE,
            PaperStatus.DENIED,
        ):
            return Response(
                {"detail": ValidationError(_("완료 또는 거절된 계약서는 서명을 수정할 수 없습니다."))},
                status=status.HTTP_400_BAD_REQUEST,
            )
        return self.update(request, *args, **kwargs)


class PaperLoadAPIView(mixins.RetrieveModelMixin, generics.GenericAPIView):
...
class PaperViewset(ModelViewSet):
    ...
    def get_object(self):
        obj = get_object_or_404(
            Paper.objects.select_related("author", "address", "status").prefetch_related(
                "paper_contractors",
                "paper_contractors__profile",
                "paper_contractors__profile__user",
                "paper_contractors__profile__expert_profile",
            ),
            pk=self.kwargs["pk"],
        )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if instance.status.status in (PaperStatus.DONE, PaperStatus.DENIED):
            return Response(
                {"detail": ValidationError(_("완료 또는 거절된 계약서는 수정할 수 없습니다."))},
                status=status.HTTP_400_BAD_REQUEST,
            )
        elif instance.status.status == PaperStatus.PROGRESS:
            initial_date = datetime.datetime(9999, 12, 31)
            signature_first_updated_at = getattr(
                Signature.objects.order_by("updated_at")
                .filter(contractor__paper=instance, updated_at__gte=instance.updated_at)
                .first(),
                "updated_at",
                initial_date,
            )
            ...
            explanation_signature_first_updated_at = getattr(
                ExplanationSignature.objects.order_by("updated_at")
                .filter(contractor__paper=instance, updated_at__gte=instance.updated_at)
                .first(),
                "updated_at",
                initial_date,
            )
            explanation_signature_first_updated_at = (
                explanation_signature_first_updated_at.replace(tzinfo=None)
            )
            first_signature_time = (
                signature_first_updated_at
                if signature_first_updated_at <= explanation_signature_first_updated_at
                else explanation_signature_first_updated_at
            )
            if first_signature_time != initial_date:
                if (datetime.datetime.utcnow() - first_signature_time).total_seconds() / 3600 > 24:
                    return Response(
                        {"detail": ValidationError(_("최초 서명 후 24시간이 지나면 계약서를 수정 할 수 없습니다."))},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.status.status == PaperStatus.DONE:
            return Response(
                {"detail": ValidationError(_("완료된 계약서는 삭제할 수 없습니다."))},
                status=status.HTTP_400_BAD_REQUEST,
            )
        elif instance.status.status == PaperStatus.PROGRESS:
            initial_date = datetime.datetime(9999, 12, 31)
            ...
            if first_signature_time != initial_date:
                if (datetime.datetime.utcnow() - first_signature_time).total_seconds() / 3600 > 24:
                    return Response(
                        {"detail": ValidationError(_("최초 서명 후 24시간이 지나면 계약서를 삭제 할 수 없습니다."))},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
